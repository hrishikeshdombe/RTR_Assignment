...
GLuint gVao_Triangle,gVao_Square;
GLuint gVbo_Pos,gVbo_Color;
GLuint gMVPUniform;

GLfloat gAngle_Square,gAngle_Triangle;
...
void initialize(void)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER,0);
	glVertexAttrib3f(HAD_ATTRIBUTE_COLOR,0.39f,0.58f,0.93f);
...
void display(void)
{
	...
	rotationMatrix=rotate(gAngle_Triangle,0.0f,1.0f,0.0f);
	...
	rotationMatrix=rotate(gAngle_Square,1.0f,0.0f,0.0f);
	modelViewMatrix=modelViewMatrix*rotationMatrix;
...
void update(void)
{
	...
	if(gAngle_Triangle>=360.0f)
		gAngle_Triangle=gAngle_Triangle-360.0f;
