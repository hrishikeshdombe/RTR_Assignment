...
void display(void)
{
	...
	worldMatrix = rotationMatrix_Y * translationMatrix * worldMatrix;
	...
	if (gbLight == true)
	{
		...
		fopen_s(&gpFile, gszLogFileName, "a");
		fprintf_s(gpFile, "gbLight = true ulkeypressed : %d\n", constantBuffer.u_lkeypressed);
		fclose(gpFile);
	}
	else
	{
		...
		fopen_s(&gpFile, gszLogFileName, "a");
		fprintf_s(gpFile, "gbLight = false ulkeypressed : %d\n", constantBuffer.u_lkeypressed);
		fclose(gpFile);
	}
